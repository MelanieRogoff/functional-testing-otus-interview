...
async function folderNames(driver) {
    ...
    const doneFolder = await driver.findElement(By.xpath("//div[@class='reveal otus-modal modal-fixed-textview otus-modal-top otus-modal-top-radius']//div[@class='nav-right']//button[@class='btn btn-light small-size ng-star-inserted']"));
    await multiClick(driver, doneFolder, 1); //First entry
    ...
    await driver.sleep(4000);
    await multiClick(driver, doneFolder, 1); //Second entry
    ...
    await driver.findElement(By.xpath("//input[@placeholder='Folder's Name...']")).sendKeys('ABC123!?', Key.RETURN);
    await driver.sleep(4000);
    await multiClick(driver, doneFolder, 1); //Final entry
}
